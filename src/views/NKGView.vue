      ...
      <div class="container">
        ...
        <div class="timeline" ref="timeline" :class="{ 'animate-in': isVisible3 }">
          <div
            ...
            :title="item.isClickable ? '点击观看夺冠纪念视频' : undefined"
            :style="item.isClickable ? 'cursor: pointer;' : undefined"
            @click="item.isClickable ? openChampionVideo() : undefined"
          >
            <div class="timeline-year">{{ item.year }}</div>
            <div class="timeline-content">
              <h3>{{ item.title }}</h3>
              <p>{{ item.desc }}</p>
              <div v-if="item.clickHint" class="click-hint">{{ item.clickHint }}</div>
            </div>
          </div>
        </div>
      </div>
...
.nkg-page .hero-background {
  background:
    radial-gradient(circle at 14% 14%, rgba(253, 232, 233, 0.72), transparent 20%),
    radial-gradient(circle at 84% 18%, rgba(188, 158, 193, 0.24), transparent 24%),
    var(--hero-bg);
}
